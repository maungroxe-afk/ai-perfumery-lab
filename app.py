...
# --- KONFIGURASI HALAMAN ---                                                                                 
  ...
  # --- TABS ---                                                                                                
  tab_formula, tab_ai, tab_db = st.tabs(["⚖️ Kalkulator Formulasi", "🤖 AI Perfumer Assistant", "📚 Database    
IFRA Pro"])                                                                                                     
  ...
  with tab_formula:                                                                                             
      ...
      if not df_ifra.empty:                                                                                     
          ...
          if st.session_state.formula:                                                                          
              ...
              df_formula = pd.merge(df_formula, df_ifra[["Bahan", "Kategori_IFRA_4", "Tipe_Note"]], on="Bahan", 
how="left")                                                                                                     
              ...
              df_formula.rename(columns={"Kategori_IFRA_4": "Batas IFRA (%)", "Tipe_Note": "Tipe Note"},        
inplace=True)                                                                                                   
                                                                                                                
              # Menyusun urutan kolom agar lebih enak dibaca                                                    
              df_tampil = df_formula[["Bahan", "Tipe Note", "Jumlah (g)", "Persentase (%)", "Batas IFRA (%)",   
"Status IFRA"]]                                                                                                 
                                                                                                                
              st.subheader("Detail Formulasi Anda")                                                             
              st.write(f"**Total Berat Formula:** {total_jumlah:.2f} Gram")                                     
              st.dataframe(df_tampil.style.map(lambda x: "background-color: #ffcccc" if "❌" in str(x) else     
"background-color: #ccffcc" if "✅" in str(x) else "", subset=["Status IFRA"]))                                 
  ...
  with tab_ai:                                                                                                  
      ...
      prompt_user = st.text_area("Tanyakan sesuatu ke AI... (Contoh: 'Buatkan saya formula parfum floral musky  
dengan 5 bahan')")                                                                                              
          ...
          if not api_key:                                                                                       
              ...
              try:                                                                                              
                  ...
                  with st.spinner("AI sedang memikirkan racikan terbaik..."):                                   
                      ...
                      konteks_system = "Kamu adalah seorang Master Perfumer yang sangat ahli dalam meracik      
parfum dan hafal standar IFRA Kategori 4. "                                                                     
  ...
  with tab_db:                                                                                                  
      ...
      st.write("Diambil otomatis dari file `database_ifra_pro.csv`. Anda bisa memperbarui file ini kapan saja di
GitHub tanpa mengubah kode aplikasi.")                                                                          
      if not df_ifra.empty:                                                                                     
